...
static int is_well_surrounded(char **map, int i, int j)
{
    if (i == 0 || j == 0 || j == (int)ft_strlen(map[i]) - 1 || !map[i + 1])
        return (-1);
    if (is_spaces(map[i][j - 1]) || is_spaces(map[i][j + 1]))
        return (-1);
    if (is_spaces(map[i + 1][j]) || is_spaces(map[i - 1][j]))
        return (-1);
    if (is_spaces(map[i - 1][j - 1]) || is_spaces(map[i - 1][j + 1]))
        return (-1);
    if (is_spaces(map[i + 1][j - 1]) || is_spaces(map[i + 1][j + 1]))
        return (-1);
    return (0);
...
static int  is_cell_valid(char **map, int i, int j, int *spawn_found)
{
    ...
    if (is_spawn(map[i][j]))
        (*spawn_found)++;
    if (map[i][j] == '1' || is_spaces(map[i][j]))
        return (0);
